# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  caddy-l4 = {
    pname = "caddy-l4";
    version = "v0.1.1";
    src = fetchFromGitHub {
      owner = "mholt";
      repo = "caddy-l4";
      rev = "v0.1.1";
      fetchSubmodules = false;
      sha256 = "sha256-P6ltBzCQLELDtIQkQ2E3xoNBjCyLvwCoWWztyRoR7ZI=";
    };
  };
  claude-code = {
    pname = "claude-code";
    version = "2.1.172";
    src = fetchurl {
      url = "https://registry.npmjs.org/@anthropic-ai/claude-code/-/claude-code-2.1.172.tgz";
      sha256 = "sha256-nZnN0sw1Z44YyUruyNI+b3oZQ8tPRTgqZfWodiPiPvw=";
    };
  };
  deploy-rs = {
    pname = "deploy-rs";
    version = "2ce9051767ee4d1a3c43b52ba327431783bfd463";
    src = fetchFromGitHub {
      owner = "serokell";
      repo = "deploy-rs";
      rev = "2ce9051767ee4d1a3c43b52ba327431783bfd463";
      fetchSubmodules = false;
      sha256 = "sha256-Gt+qFANcrDRjl3xzidLYrAUQCd3808iuAsLwZbYYAEU=";
    };
    date = "2026-06-09";
  };
  netbird = {
    pname = "netbird";
    version = "v0.72.3";
    src = fetchFromGitHub {
      owner = "netbirdio";
      repo = "netbird";
      rev = "v0.72.3";
      fetchSubmodules = true;
      sha256 = "sha256-osWOVV8nX5OC6YFO8lRO+q/IoEth1wZhrukkZZk3K+0=";
    };
  };
  oh-my-zsh = {
    pname = "oh-my-zsh";
    version = "c954bbb168fc645592c50017de0d0e138db8df5f";
    src = fetchFromGitHub {
      owner = "ohmyzsh";
      repo = "ohmyzsh";
      rev = "c954bbb168fc645592c50017de0d0e138db8df5f";
      fetchSubmodules = false;
      sha256 = "sha256-3CGIN2b/lLI+rB76lbhFugvdu/rj9n/8sV3IlVexgco=";
    };
    date = "2026-06-10";
  };
  openbao = {
    pname = "openbao";
    version = "v2.5.4";
    src = fetchFromGitHub {
      owner = "openbao";
      repo = "openbao";
      rev = "v2.5.4";
      fetchSubmodules = false;
      sha256 = "sha256-Gf/2/3XLzCnmU8hJfctUra6mamBsH/4QEiC89FolKQ4=";
    };
  };
  remotepc-host = {
    pname = "remotepc-host";
    version = "6.12.0";
    src = fetchurl {
      url = "https://static.remotepc.com/downloads/rpc/310320/remotepc-host.deb";
      sha256 = "sha256-mVDKeUeFba2KtIaVCBdiQ/R/IV/rvtWXox1A+zyPK6c=";
    };
  };
  remotepc-host-pi64 = {
    pname = "remotepc-host-pi64";
    version = "6.10.3";
    src = fetchurl {
      url = "https://static.remotepc.com/downloads/rpc/310320/remotepc-host-pi64.deb";
      sha256 = "sha256-GWudSHJLok4ZnedN2SFZN5+x42NcV6f3GfKPld/mVhA=";
    };
  };
  rustdesk = {
    pname = "rustdesk";
    version = "1.4.7";
    src = fetchFromGitHub {
      owner = "rustdesk";
      repo = "rustdesk";
      rev = "1.4.7";
      fetchSubmodules = true;
      sha256 = "sha256-k4hJjr6xN1EOPfxRoTZMAUXZCFrwod3c+jVtBbqqO0U=";
    };
  };
  vault = {
    pname = "vault";
    version = "v2.0.2";
    src = fetchFromGitHub {
      owner = "hashicorp";
      repo = "vault";
      rev = "v2.0.2";
      fetchSubmodules = false;
      sha256 = "sha256-EebBguA9YuqtrXTcKHNwjQv7bgXFBtaRN0+lk7Zn6n0=";
    };
  };
  vector = {
    pname = "vector";
    version = "v0.56.0";
    src = fetchFromGitHub {
      owner = "vectordotdev";
      repo = "vector";
      rev = "v0.56.0";
      fetchSubmodules = false;
      sha256 = "sha256-ge3epfB8xErF+2I1jW3OvHS+mHnGSSU6vOz2v/sSMW4=";
    };
  };
  victoriametrics-cluster = {
    pname = "victoriametrics-cluster";
    version = "v1.145.0-cluster";
    src = fetchFromGitHub {
      owner = "VictoriaMetrics";
      repo = "VictoriaMetrics";
      rev = "v1.145.0-cluster";
      fetchSubmodules = false;
      sha256 = "sha256-7s8dkYMMwsnOdLvF2ikuVxB/ifKGkTGMyPlnNlpvRTo=";
    };
  };
  zabbix74 = {
    pname = "zabbix74";
    version = "7.4.11";
    src = fetchFromGitHub {
      owner = "zabbix";
      repo = "zabbix";
      rev = "7.4.11";
      fetchSubmodules = false;
      sha256 = "sha256-i0zWbDI5RAvPfPuWVyz0ZbAbynRwYugupwmShHiNQwQ=";
    };
  };
  zabbix80pre = {
    pname = "zabbix80pre";
    version = "8.0.0beta1";
    src = fetchFromGitHub {
      owner = "zabbix";
      repo = "zabbix";
      rev = "8.0.0beta1";
      fetchSubmodules = false;
      sha256 = "sha256-hEhposPj3CEZt/RSnfuVGsQmvFtWyghfIRi0dygHBKk=";
    };
  };
}
