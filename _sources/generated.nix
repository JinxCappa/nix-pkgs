# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  claude-code = {
    pname = "claude-code";
    version = "2.1.105";
    src = fetchurl {
      url = "https://registry.npmjs.org/@anthropic-ai/claude-code/-/claude-code-2.1.105.tgz";
      sha256 = "sha256-tCAgaU/b3SFqElflrv572JOAPHRFT4cohPsJtnXK+4g=";
    };
  };
  deploy-rs = {
    pname = "deploy-rs";
    version = "77c906c0ba56aabdbc72041bf9111b565cdd6171";
    src = fetchFromGitHub {
      owner = "serokell";
      repo = "deploy-rs";
      rev = "77c906c0ba56aabdbc72041bf9111b565cdd6171";
      fetchSubmodules = false;
      sha256 = "sha256-hwsYgDnby50JNVpTRYlF3UR/Rrpt01OrxVuryF40CFY=";
    };
    date = "2026-02-02";
  };
  netbird = {
    pname = "netbird";
    version = "v0.68.2";
    src = fetchFromGitHub {
      owner = "netbirdio";
      repo = "netbird";
      rev = "v0.68.2";
      fetchSubmodules = true;
      sha256 = "sha256-q4EaiXnl7LEZ3fsQLhOzNutQUk0Q/sFuNtz66AKiGJQ=";
    };
  };
  oh-my-zsh = {
    pname = "oh-my-zsh";
    version = "061f773dd356df52a8bccd5e73377c012f97ef14";
    src = fetchFromGitHub {
      owner = "ohmyzsh";
      repo = "ohmyzsh";
      rev = "061f773dd356df52a8bccd5e73377c012f97ef14";
      fetchSubmodules = false;
      sha256 = "sha256-aK3iX+YweXr+LCQJYfzP0M049ujyoE+MVotKv+WccBg=";
    };
    date = "2026-04-13";
  };
  openbao = {
    pname = "openbao";
    version = "v2.5.2";
    src = fetchFromGitHub {
      owner = "openbao";
      repo = "openbao";
      rev = "v2.5.2";
      fetchSubmodules = false;
      sha256 = "sha256-fqsCQf9wFlzMWC4XV1BclWlYG649VhhZBKlGrdsbqKc=";
    };
  };
  remotepc-host = {
    pname = "remotepc-host";
    version = "6.10.5";
    src = fetchurl {
      url = "https://download.remotepc.com/downloads/rpc/310320/remotepc-host.deb";
      sha256 = "sha256-rn98QQHISqO1XaPvI9rJkTwI6TdplELXftULW0ccST4=";
    };
  };
  rustdesk = {
    pname = "rustdesk";
    version = "1.4.6";
    src = fetchFromGitHub {
      owner = "rustdesk";
      repo = "rustdesk";
      rev = "1.4.6";
      fetchSubmodules = true;
      sha256 = "sha256-2MZOM+SHDrjFhCIHcFB7zABpwC7hNtS0XNFx2FpaqIE=";
    };
  };
  vault = {
    pname = "vault";
    version = "v1.21.4";
    src = fetchFromGitHub {
      owner = "hashicorp";
      repo = "vault";
      rev = "v1.21.4";
      fetchSubmodules = false;
      sha256 = "sha256-1yBvcGKzLZYFWlZJL1iJgDFkiT4g2f84iZCjWi2CwDg=";
    };
  };
  vector = {
    pname = "vector";
    version = "v0.54.0";
    src = fetchFromGitHub {
      owner = "vectordotdev";
      repo = "vector";
      rev = "v0.54.0";
      fetchSubmodules = false;
      sha256 = "sha256-XEQN6Dk72Yn3FZoqvQk4mj+ME68hJuN7zHpiSLY9iNo=";
    };
  };
  victoriametrics-cluster = {
    pname = "victoriametrics-cluster";
    version = "v1.140.0-cluster";
    src = fetchFromGitHub {
      owner = "VictoriaMetrics";
      repo = "VictoriaMetrics";
      rev = "v1.140.0-cluster";
      fetchSubmodules = false;
      sha256 = "sha256-v31/OVmzZAd+HXVScd4UBrtP0yCyeJjcyEK3eukN5hE=";
    };
  };
  zabbix74 = {
    pname = "zabbix74";
    version = "7.4.9";
    src = fetchFromGitHub {
      owner = "zabbix";
      repo = "zabbix";
      rev = "7.4.9";
      fetchSubmodules = false;
      sha256 = "sha256-ZW+cHR01m5XQgtzYJ9o+KG2iQzLNKK6b2hh34tdT4F8=";
    };
  };
}
