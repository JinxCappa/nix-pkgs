# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  caddy-l4 = {
    pname = "caddy-l4";
    version = "v0.1.2";
    src = fetchFromGitHub {
      owner = "mholt";
      repo = "caddy-l4";
      rev = "v0.1.2";
      fetchSubmodules = false;
      sha256 = "sha256-xys5S1QAGGHF/XHXWxjfjeplVr1Aq1wks37lRNJkmKM=";
    };
  };
  claude-code = {
    pname = "claude-code";
    version = "2.1.278";
    src = fetchurl {
      url = "https://registry.npmjs.org/@anthropic-ai/claude-code/-/claude-code-2.1.278.tgz";
      sha256 = "sha256-CMbfzz2vz9MOCbKSbFluJ08PoghEpYAa2n8cjmInFX4=";
    };
  };
  deploy-rs = {
    pname = "deploy-rs";
    version = "e760371d631165e7d8de5b0dcf148e21ec4c16f0";
    src = fetchFromGitHub {
      owner = "serokell";
      repo = "deploy-rs";
      rev = "e760371d631165e7d8de5b0dcf148e21ec4c16f0";
      fetchSubmodules = false;
      sha256 = "sha256-UXFQ7tFiwn8sPz0EV4CBB2PCf/ZiGIHWn/6MXk81Lxs=";
    };
    date = "2026-09-14";
  };
  netbird = {
    pname = "netbird";
    version = "v0.79.0";
    src = fetchFromGitHub {
      owner = "netbirdio";
      repo = "netbird";
      rev = "v0.79.0";
      fetchSubmodules = true;
      sha256 = "sha256-Bi83uh8VKvFGUY+AxP34VCXYxpZI1C/oOa6eHmKXpDw=";
    };
  };
  oh-my-zsh = {
    pname = "oh-my-zsh";
    version = "6421f8e104e4e87f2373362cbf61e46a918612a7";
    src = fetchFromGitHub {
      owner = "ohmyzsh";
      repo = "ohmyzsh";
      rev = "6421f8e104e4e87f2373362cbf61e46a918612a7";
      fetchSubmodules = false;
      sha256 = "sha256-vBIZtWwRgWN9kEkcLyLHNRPhs6TNWMpXYAaLq6n2S8Y=";
    };
    date = "2026-09-20";
  };
  openbao = {
    pname = "openbao";
    version = "v2.6.2";
    src = fetchFromGitHub {
      owner = "openbao";
      repo = "openbao";
      rev = "v2.6.2";
      fetchSubmodules = false;
      sha256 = "sha256-iFOah7LkuiL4MGhi6ANpgCYDYQl33gF+F4agdKLEFTk=";
    };
  };
  remotepc-host = {
    pname = "remotepc-host";
    version = "6.12.3";
    src = fetchurl {
      url = "https://static.remotepc.com/downloads/rpc/310320/remotepc-host.deb";
      sha256 = "sha256-4YmRzC6lNfD4Id1V6ERv9XBpvyvEw6jGrAzatB3XP1c=";
    };
  };
  remotepc-host-pi64 = {
    pname = "remotepc-host-pi64";
    version = "6.10.3";
    src = fetchurl {
      url = "https://static.remotepc.com/downloads/rpc/310320/remotepc-host-pi64.deb";
      sha256 = "sha256-GWudSHJLok4ZnedN2SFZN5+x42NcV6f3GfKPld/mVhA=";
    };
  };
  rustdesk = {
    pname = "rustdesk";
    version = "1.4.9";
    src = fetchFromGitHub {
      owner = "rustdesk";
      repo = "rustdesk";
      rev = "1.4.9";
      fetchSubmodules = true;
      sha256 = "sha256-AnwdIO4TveC48uMioBCvH60xun24ckK420ONSEB9lQI=";
    };
  };
  sops-install-secrets = {
    pname = "sops-install-secrets";
    version = "7214124c20c1542c90deb54af50e2f53ae02711f";
    src = fetchFromGitHub {
      owner = "Mic92";
      repo = "sops-nix";
      rev = "7214124c20c1542c90deb54af50e2f53ae02711f";
      fetchSubmodules = false;
      sha256 = "sha256-GKwH3zpy7tartuJMG0Rv/xUsdetG1QLmTVv8UKgJLmA=";
    };
    date = "2026-09-20";
  };
  technitium-dns-server = {
    pname = "technitium-dns-server";
    version = "15.5.0";
    src = fetchurl {
      url = "https://github.com/TechnitiumSoftware/DnsServer/archive/refs/tags/v15.5.0.tar.gz";
      sha256 = "sha256-E1dC41l5g00zKam7sKZc0BRcU6XGSsMxpeQ9+XB2aD4=";
    };
  };
  technitium-dns-server-library = {
    pname = "technitium-dns-server-library";
    version = "15.5.0";
    src = fetchurl {
      url = "https://github.com/TechnitiumSoftware/TechnitiumLibrary/archive/refs/tags/dns-server-v15.5.0.tar.gz";
      sha256 = "sha256-yPY1w+Ry2uZ36RBExuQA2LOdZZNl9ACvj1Yw+poJwgw=";
    };
  };
  vault = {
    pname = "vault";
    version = "v2.1.1";
    src = fetchFromGitHub {
      owner = "hashicorp";
      repo = "vault";
      rev = "v2.1.1";
      fetchSubmodules = false;
      sha256 = "sha256-pbPK3pFAlBCoioJNZaCI61GxFZt6SeCmn+8yoVmUJe4=";
    };
  };
  vector = {
    pname = "vector";
    version = "v0.58.0";
    src = fetchFromGitHub {
      owner = "vectordotdev";
      repo = "vector";
      rev = "v0.58.0";
      fetchSubmodules = false;
      sha256 = "sha256-H/bSlSWdNN94uCP0tpjyf/VEdoCb/PUALTknT/UNdfg=";
    };
  };
  victoriametrics-cluster = {
    pname = "victoriametrics-cluster";
    version = "v1.151.0-cluster";
    src = fetchFromGitHub {
      owner = "VictoriaMetrics";
      repo = "VictoriaMetrics";
      rev = "v1.151.0-cluster";
      fetchSubmodules = false;
      sha256 = "sha256-VK5bzIldzafQKJR2snj0mxk1YZUf0rhavtXWCQRlqH4=";
    };
  };
  zabbix74 = {
    pname = "zabbix74";
    version = "7.4.14";
    src = fetchFromGitHub {
      owner = "zabbix";
      repo = "zabbix";
      rev = "7.4.14";
      fetchSubmodules = false;
      sha256 = "sha256-hO10/BagyT3txxH+IwWSlRPrOK9Rn31tmRL4RqJHh1E=";
    };
  };
  zabbix80pre = {
    pname = "zabbix80pre";
    version = "8.0.0beta2";
    src = fetchFromGitHub {
      owner = "zabbix";
      repo = "zabbix";
      rev = "8.0.0beta2";
      fetchSubmodules = false;
      sha256 = "sha256-gtfmFlqhLBmmYiyzgLq9GVPOV1KDco5LXZ84msyNBwM=";
    };
  };
}
