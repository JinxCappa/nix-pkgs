# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  claude-code = {
    pname = "claude-code";
    version = "2.1.61";
    src = fetchurl {
      url = "https://registry.npmjs.org/@anthropic-ai/claude-code/-/claude-code-2.1.61.tgz";
      sha256 = "sha256-PhOg2Vx5AUpR0oAlo9YcPO0bsb+5JRuGoGLgsJTsIQs=";
    };
  };
  deploy-rs = {
    pname = "deploy-rs";
    version = "77c906c0ba56aabdbc72041bf9111b565cdd6171";
    src = fetchFromGitHub {
      owner = "serokell";
      repo = "deploy-rs";
      rev = "77c906c0ba56aabdbc72041bf9111b565cdd6171";
      fetchSubmodules = false;
      sha256 = "sha256-hwsYgDnby50JNVpTRYlF3UR/Rrpt01OrxVuryF40CFY=";
    };
    date = "2026-02-02";
  };
  netbird = {
    pname = "netbird";
    version = "v0.66.0";
    src = fetchFromGitHub {
      owner = "netbirdio";
      repo = "netbird";
      rev = "v0.66.0";
      fetchSubmodules = true;
      sha256 = "sha256-Kp/doDg9gNaGpsQYkmY+vghznxeh0htU5m7UC6fhKvc=";
    };
  };
  oh-my-zsh = {
    pname = "oh-my-zsh";
    version = "5c4f27b7166360bd23709f24642b247eac30a147";
    src = fetchFromGitHub {
      owner = "ohmyzsh";
      repo = "ohmyzsh";
      rev = "5c4f27b7166360bd23709f24642b247eac30a147";
      fetchSubmodules = false;
      sha256 = "sha256-ipjFO5qOEXnLG+xkB1UdkkukYs+9pGlW4vummCTJQ1s=";
    };
    date = "2026-02-25";
  };
  openbao = {
    pname = "openbao";
    version = "v2.5.1";
    src = fetchFromGitHub {
      owner = "openbao";
      repo = "openbao";
      rev = "v2.5.1";
      fetchSubmodules = false;
      sha256 = "sha256-pVFsyNg9ccSFAdHb/fjeVoMBh1nKcjwcFfVBBqFalIo=";
    };
  };
  rustdesk = {
    pname = "rustdesk";
    version = "1.4.5";
    src = fetchFromGitHub {
      owner = "rustdesk";
      repo = "rustdesk";
      rev = "1.4.5";
      fetchSubmodules = true;
      sha256 = "sha256-FRtYafsIKHnGPV8NaiaHxIHkon8/T2P83uq9taUD1Xc=";
    };
  };
  vault = {
    pname = "vault";
    version = "v1.21.2";
    src = fetchFromGitHub {
      owner = "hashicorp";
      repo = "vault";
      rev = "v1.21.2";
      fetchSubmodules = false;
      sha256 = "sha256-G/6GroX9lBaUxB5mkd+hz53nNX/eF5DAtHryin3hzgU=";
    };
  };
  vector = {
    pname = "vector";
    version = "vdev-v0.2.0";
    src = fetchFromGitHub {
      owner = "vectordotdev";
      repo = "vector";
      rev = "vdev-v0.2.0";
      fetchSubmodules = false;
      sha256 = "sha256-mkogl+HOV/nrmvqUuQFyMEzwDfYHGumF9jevXIbNLuk=";
    };
  };
  victoriametrics-cluster = {
    pname = "victoriametrics-cluster";
    version = "v1.136.0-cluster";
    src = fetchFromGitHub {
      owner = "VictoriaMetrics";
      repo = "VictoriaMetrics";
      rev = "v1.136.0-cluster";
      fetchSubmodules = false;
      sha256 = "sha256-UczQQ7mdzYh7ZHuqHF3+3a3Wa3hDCn2gIF77uRrs5RM=";
    };
  };
  zabbix74 = {
    pname = "zabbix74";
    version = "7.4.7";
    src = fetchFromGitHub {
      owner = "zabbix";
      repo = "zabbix";
      rev = "7.4.7";
      fetchSubmodules = false;
      sha256 = "sha256-Q+Bf+3KYNIY8wHuVMMZ5u2zFnjyrba6VG1KYqB29Ikw=";
    };
  };
}
